...
struct PoetTagController: RouteCollection {
    
    func boot(routes: RoutesBuilder) throws {
        let poets = routes.grouped("poets", "tag")
    ...
    /// 添加标签
    func attachPoetTag(req: Request) throws -> EventLoopFuture<Void> {
        
        let poetTang = PoetTang.find(req.parameters.get("id"), on: req.db).unwrap(or: Abort(.notFound))
        let poetTag = PoetTag.find(req.parameters.get("id"), on: req.db).unwrap(or: Abort(.notFound))
        
        return poetTang.and(poetTag).flatMap { (tang, tag) -> EventLoopFuture<Void> in
            let info = tang.$tags.attach(tag, on: req.db)
            return info
        }
    }
    /// 移除标签
    func detachPoetTag(req: Request) throws -> EventLoopFuture<Void> {
        let poetTang = PoetTang.find(req.parameters.get("id"), on: req.db).unwrap(or: Abort(.notFound))
        let poetTag = PoetTag.find(req.parameters.get("id"), on: req.db).unwrap(or: Abort(.notFound))
        
        return poetTang.and(poetTag).flatMap { (tang, tag) -> EventLoopFuture<Void> in
            let info = tang.$tags.detach(tag, on: req.db)
            return info
        }
    }
    /// 获取标签
    func getPoetTag(req: Request) throws -> EventLoopFuture<Void> {
        let poetTang = PoetTang.find(req.parameters.get("id"), on: req.db).unwrap(or: Abort(.notFound))
        let poetTag = PoetTag.find(req.parameters.get("id"), on: req.db).unwrap(or: Abort(.notFound))
